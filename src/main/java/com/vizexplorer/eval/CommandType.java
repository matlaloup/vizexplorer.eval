...
/**
 * Enum class for supported commands
 * 
 */
public enum CommandType
{
  ...
  private CommandType(String argName)
  {
    if (argName == null)
    {
  ...
  /**
   * Returns the corresponding action. Null if no action matches the argument.
   * 
   * @param argName
   * @return
   */
  public static CommandType resolve(String argName)
  {
    for (CommandType action : values())
    {
      if (action.getArgName().equals(argName))
      {
        return action;
      }
    }
